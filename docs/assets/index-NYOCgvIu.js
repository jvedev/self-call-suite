...
`;class o extends HTMLElement{constructor(){super(),this.abortController=new AbortController,this.signal=this.abortController.signal,this.eventCleanup={signal:this.signal};const t=new CSSStyleSheet;t.replaceSync(c),this.attachShadow({mode:"open"}),this.root.adoptedStyleSheets=[t]}get placeHolder(){return this.queryRoot(".placeholder")}getParentHost(){return this.root.host}get root(){if(!this.shadowRoot)throw new Error("SubProcedureToken has no shadowRoot.");return this.shadowRoot}disconnectedCallback(){this.abortController.abort(),this.resizeObserver?.disconnect()}render(t="",e=""){this.root.innerHTML=`<style>
                ${t}
             </style>
             ${e}`}queryRoot(t){const e=this.root.querySelector(t);if(!e)throw new Error(`Element not found for query ${t}`);return e}dispatchCustomEvent(t,e){this.dispatchEvent(new CustomEvent(t,{detail:e}))}queryRootAll(t){return this.root.querySelectorAll(t)}}class d extends HTMLElement{static get observedAttributes(){return["path"]}get path(){return this.getAttribute("path")||""}connectedCallback(){this.style.display="none"}}class u extends o{render(){throw new Error("Method not implemented.")}attachEventListeners(){throw new Error("Method not implemented.")}removeEventListeners(){throw new Error("Method not implemented.")}constructor(){super(),this.observer=new MutationObserver(()=>this.updateRoute())}connectedCallback(){window.addEventListener("popstate",()=>this.updateRoute()),this.observer.observe(this,{childList:!0,subtree:!0}),this.updateRoute()}disconnectedCallback(){window.removeEventListener("popstate",()=>this.updateRoute()),this.observer.disconnect()}updateRoute(){const t=window.location.pathname;let e=!1;if(Array.from(this.querySelectorAll("view-route, .route-404")).forEach(r=>{r.path===t?(r.style.display="",e=!0):r.style.display="none"}),e)return e;this.show404()}show404(){let t=this.querySelector(".route-404");t||(t=document.createElement("div"),t.textContent="404 Not Found",t.className="route-404",this.appendChild(t)),t.style.display=""}}customElements.define("app-router",u);customElements.define("view-route",d);const h=`<div class="placeholder" mode="stop">\r
    ...
    <div class="controls">\r
        ...
        <button class="pause">Pause</button>\r
...
\r
\r
`;class f extends o{constructor(){super(),this._warnings=[],this.render(v,w)}get warning(){if(!this._warning)throw new Error("No warning selected");return this._warning}set warning(t){this._warning=t}set warnings(t){this._warnings=t,this.renderWarnings()}connectedCallback(){}renderWarnings(){const{placeHolder:t}=this;t.innerHTML="",this._warnings.forEach(e=>{const n=document.createElement("div");n.classList.add("warning-option"),n.innerHTML=e.description,t.appendChild(n),n.addEventListener("click",()=>this.warningSelected(e))})}warningSelected(t){console.log(`Warning selected: ${JSON.stringify(t)}`),this._warning=t,this.dispatchEvent(new CustomEvent("warning-selected",{detail:{warning:t}}))}}customElements.define("warning-selector",f);const k=`<div class="placeholder" >\r
\r
</div>\r
\r
`,E=`:host {\r
...
\r
\r
`;class S extends o{constructor(){super(),this.penalty=0,this.render(E,k)}set warning(t){this._warning=t,this.renderDeductionButtons()}get warning(){if(!this._warning)throw new Error("Warning is not set");return this._warning}get placeholder(){return this.queryRoot(".placeholder")}onSelect(t){this.penalty=t,this.dispatchEvent(new CustomEvent("deduction-selected",{detail:{penalty:t},bubbles:!0,composed:!0}))}renderDeductionButtons(){this.placeholder.innerHTML="";const{minPenalty:t,maxPenalty:e}=this.warning,n=document.createElement("button");n.addEventListener("click",()=>this.onSelect(0)),n.className="no-penalty button green",n.innerText="No penalty",this.placeholder.appendChild(n);for(let r=t;r<=e;r++){const i=document.createElement("button");i.textContent=r.toString(),i.className="red button",i.addEventListener("click",()=>this.onSelect(r)),this.placeholder.appendChild(i)}}}customElements.define("warning-deduction",S);const x=`<div class="placeholder" >\r
...
\r
\r
`;class L extends o{constructor(){super(),this._player=void 0,this.render(R,x)}set player(t){this._player=t,this.dispatchEvent(new CustomEvent("player-selected",{detail:{player:t}}))}get player(){return this._player}connectedCallback(){this.queryRoot("button.red").addEventListener("click",()=>this.player="red"),this.queryRoot("button.blue").addEventListener("click",()=>this.player="blue")}}customElements.define("warning-player",L);const C=`<div class="placeholder" >\r
...
\r
\r
`;class q extends o{set warning(t){this.queryRoot(".warning").innerHTML=t.description}set player(t){const e=this.queryRoot(".player");e.innerHTML=t||"",e.className=`player text-${t}`}set penalty(t){this.queryRoot(".penalty").innerHTML=t.toString()}get placeholder(){return this.queryRoot(".placeholder")}constructor(){super(),this.render(B,C)}}customElements.define("warning-summery",q);const O=`<div class="placeholder">\r
...
\r
\r
`;class $ extends o{constructor(){super(),this.settings={calloutOnScore:!0},this.render(P,T)}set playerRed(t){const e=this.queryRoot(".player.red .name");e.textContent=t}set playerBlue(t){const e=this.queryRoot(".player.blue .name");e.textContent=t}set scoreRed(t){const e=this.queryRoot(".player.red .score");e.textContent=t.toString()}get scoreRed(){const t=this.queryRoot(".player.red .score");return parseInt(t.textContent)}set scoreBlue(t){const e=this.queryRoot(".player.blue .score");e.textContent=t.toString()}get scoreBlue(){const t=this.queryRoot(".player.blue .score");return parseInt(t.textContent)}get main(){return this.queryRoot(".main-view")}get timer(){return this.queryRoot("timer-component")}get score(){return this.queryRoot("score-component")}get warning(){return this.queryRoot("warning-component")}get callOut(){return this.queryRoot("call-out")}get hitButton(){return this.queryRoot("#hit")}get warningButton(){return this.queryRoot("#warning")}get timeoutButton(){return this.queryRoot("#timeout")}get extentButton(){return this.queryRoot("#extent")}get stopButton(){return this.queryRoot("#stop")}connectedCallback(){this.timeoutButton.addEventListener("click",this.pause.bind(this),this.eventCleanup),this.hitButton.addEventListener("click",this.showScore.bind(this),this.eventCleanup),this.warningButton.addEventListener("click",this.showWarning.bind(this),this.eventCleanup),this.warning.addEventListener("back",this.showMain.bind(this),this.eventCleanup),this.score.addEventListener("back",this.showMain.bind(this),this.eventCleanup),this.callOut.addEventListener("click",this.showMain.bind(this),this.eventCleanup),window.addEventListener("game-event",this.gameEvent.bind(this),this.eventCleanup)}gameEvent(t){const{detail:e}=t;if(e.type=="score"){const n=e.score;if(n.scoreRed!=="low-quality"&&(this.scoreRed+=n.scoreRed),n.scoreBlue!=="low-quality"&&(this.scoreBlue+=n.scoreBlue),this.settings.calloutOnScore){this.callOut.score=n,this.showCallOut();return}}if(e.type=="warning"){const{player:n,penalty:r}=e;r&&(n=="blue"&&(this.scoreBlue-=r),n=="red"&&(this.scoreRed-=r))}this.showMain()}hideAll(){this.main.style.display="none",this.warning.style.display="none",this.score.style.display="none",this.callOut.style.display="none"}showMain(){this.hideAll(),this.main.style.display=""}showWarning(){this.hideAll(),this.warning.style.display="block"}showCallOut(){this.hideAll(),this.callOut.style.display="block"}showScore(){this.hideAll(),this.score.style.display="block"}pause(){if(this.timer.mode=="pause"){this.timer.resume(),this.timeoutButton.innerText="Timeout";return}this.timer.pause(),this.timeoutButton.innerText="Resume"}}customElements.define("match-view",$);const z=`<div class="placeholder">\r
